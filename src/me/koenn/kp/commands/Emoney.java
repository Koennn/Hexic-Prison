...
public class Emoney extends HexicCommand {
    ...
    public void onCommand(CommandSender sender, String[] args) {
        if(main.checkPlayer(args, 1, 3, sender)){
            ...
            switch (args[0]){
                case "add":
                    try{ money.addTo(p, Integer.parseInt(args[2])); }
                    catch (Exception e){ MessageManager.getInstance().msg(sender, MessageManager.MessageType.WARN, super.getUsage()); }
                    break;
                case "remove":
                    try{ money.removeFrom(p, Integer.parseInt(args[2])); }
                    catch (Exception e){ MessageManager.getInstance().msg(sender, MessageManager.MessageType.WARN, super.getUsage()); }
                    break;
                case "set":
                    try{ money.setBalance(p, Integer.parseInt(args[2])); }
                    catch (Exception e){ MessageManager.getInstance().msg(sender, MessageManager.MessageType.WARN, super.getUsage()); }
                    break;
                default:
